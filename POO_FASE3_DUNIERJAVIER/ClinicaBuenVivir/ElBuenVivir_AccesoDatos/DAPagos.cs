...
namespace ElBuenVivir_AccesoDatos
{
    public class DAPagos
    {
        ...
        // (4) Eliminar un PAGO de la base de datos
        public int EliminarPago(int pagosId)
        {
            int id = 0;
            ...
            using (SqlConnection cnx = new SqlConnection(_cadenaConexion))
            {
                ...
                SqlCommand comando = new SqlCommand();
                comando.Connection = cnx;
                string ruta = "DELETE FROM PAGOS WHERE PAGOS_ID = @PAGOS_ID";

                comando.Parameters.AddWithValue("@PAGOS_ID", pagosId);
                comando.CommandText = ruta;
                try
                {
                    cnx.Open();
