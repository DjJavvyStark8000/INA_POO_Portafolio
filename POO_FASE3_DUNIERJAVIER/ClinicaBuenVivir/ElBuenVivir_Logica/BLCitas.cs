...
namespace ElBuenVivir_Logica
{
    public class BLCitas
    {
        ...
        public List<EntidadCitas> llamarListarIDSCitas(string condicion = "")
        {
            List<EntidadCitas> cita;
            DACitas accesarCitas = new DACitas(_cadenaConexion);
            try
            {
                cita = accesarCitas.ListarIDSCitas(condicion);
            }
            catch (Exception)
            {
                throw;
        ...
        // (4) metodo para llamar al metodo eliminar citas
        public int LlamarEliminarCita(int cita)
        {
            int id_cita = 0;
            DACitas unaCita = new DACitas(_cadenaConexion);
            try
            {
                id_cita = unaCita.EliminarMiCita(cita);
            }
            catch (Exception)
            {
                throw;
